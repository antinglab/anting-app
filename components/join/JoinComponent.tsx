...
import { addCrewMember } from "@/lib/firestore/crew";
...
export default function JoinComponent({
  ...
  const typeParam = (searchParams.get("type") || "brand") as UserType;
  const resultParam = searchParams.get("result") || "none";
  ...
  const onSubmit = async (data: CrewMemberForm) => {
    setIsLoading(true);
    try {
      await addCrewMember(data, typeParam, resultParam);
      setIsSuccess(true);
    } catch (error) {
    ...
    return (
      <div className="max-w-xl mx-auto w-full text-center space-y-8 animate-fade-in py-12">
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <p className="text-white/80 text-base max-w-sm mx-auto font-light leading-relaxed">
        ...
        <div className="space-y-4 pt-6 max-w-sm mx-auto">
          <button
            onClick={handleShare}
            className="w-full bg-accent text-olive-dark font-bold rounded-full h-14 flex items-center justify-center gap-2 hover:bg-white hover:text-olive-dark transition-all duration-300 shadow-md"
          >
          ...
          <Link
            href="/landing"
            className="w-full border-2 border-white/50 text-white font-bold rounded-full h-14 flex items-center justify-center gap-2 hover:bg-white hover:text-olive-dark hover:border-white transition-all duration-300"
          >
  ...
  return (
    <div className="max-w-xl mx-auto w-full bg-white rounded-t-[24px] rounded-b-[24px] shadow-[0_4px_24px_rgba(107,124,63,0.12)] border border-olive/10 overflow-hidden">
      {/* Form Header */}
